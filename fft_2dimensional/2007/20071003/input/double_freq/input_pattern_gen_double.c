...
//initialization of the complex array
for(i=0;i<N;i++)
{
	arr[i].r = 0; arr[i].i = 0;
}
...
for(i=0;i<N;i++)
{
	//Reserve Text: Modify before uncommenting	
	////fprintf(fp1,"X(%d) = cos(k*pi*%d) + j*sin(k*pi*%d) ",i,i,i);
	

	//print values in decimal in file
	arr[i].r = (float)(0.5*cos((double)c*i*k)) + (float)(0.5*cos((double)c*i*k1));
	...
	arr[i].i = (float)(0.5*sin((double)c*i*k)) + (float)(0.5*sin((double)c*i*k1));		//SHOULD THERE BE A MINUS?
