...
printf("Now enter the components of the input signal (for simplicity they are assumed to be complex exponential signals):\n");
k = 0 ;
do
{
	printf("Enter the magnitude(between[0,1]) and frequency(between[0,1]pi) or -1 to terminate:");
	scanf("%f",&(mag[k]));
	if(mag[k]==-1)
	{
		break;
...
for (i=0;i<N;i++)
{
	...
	for(j=0;j<k;j++)
	{
		tempValue = tempValue + mag[j]*((float)sin((double)freq[j]*c*i));
	}
	tempValue = tempValue/totalMag;
	tempValue = (tempValue > ((precisionN-1)*1.0)/precisionN)?((precisionN-1)*1.0)/precisionN:tempValue;
	dec2bin((int)(tempValue*precisionN), binary,binLength);
